 ...
 * @Date: 2024/5/29
 * @Author: mengGod
 * @Description: 新能源建筑负荷类
 * 常见info打印应该使用英文，避免日志文件过大
 */
public class DeviceFuHe implements WebSocketClientUpdateByFuHe {
    private static final Logger logger = LogManager.getLogger(DeviceFuHe.class);
    ...
    @Autowired
    @Qualifier("webSocketClientByFuHe")
    @Lazy
    private WebSocketClient webSocketClientByFuHe;
    @Autowired
    ...
    public void initializeConnections() {
        ...
        try {
            ...
            redisUtils.set("deviceFuHe", jsonString);
    ...
    public void sendMessage() {
        Boolean value = Boolean.valueOf(redisUtils.get("NewOneStatusByFuHe"));
        if (value && start != 0) {
            for (DeviceInfoData deviceInfoData : deviceCredentials) {
                webSocketClientByFuHe.send("{\"type\":\"Sub\",\"topic\":\"1000/R/"+deviceInfoData.getIp()+"/Real\"}");
            }
        } else {
//            webSocketClientByFuHe.send("{\"type\":\"Verify\",\"token\":\"1000:GFKD-ZHGLPT\",\"mode\":\"CSharp\"}");
            start = 1;
        }
    }
    ...
    public void updateWebSocketClient(WebSocketClient newClient) {
        this.webSocketClientByFuHe = newClient;
    }
}
